...
use crate::x509::CrlReason;
#[cfg(ossl110)]
use crate::x509::X509Builder;
#[cfg(any(ossl110, boringssl, awslc))]
use crate::x509::X509PurposeId;
use crate::x509::{
    CrlNumber, X509CrlBuilder, X509PurposeRef, X509Ref, X509Revoked, X509RevokedBuilder,
};
use crate::x509::{
...
fn test_crl_builder() {
    ...
    let authority_key_identifier = AuthorityKeyIdentifier::new()
        ...
        .unwrap();
    crl_bldr.append_extension(authority_key_identifier).unwrap();
    let crl_number = CrlNumber::new(BigNum::from_u32(1).unwrap())
        .unwrap()
        .build()
        .unwrap();
    crl_bldr.append_extension(crl_number).unwrap();
